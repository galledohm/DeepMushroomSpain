...
- `download_images_legacy.go`: reads an iNaturalist CSV file, filters rows, and downloads images grouped by scientific name
- `go.mod`: local Go module for this directory

## Edit

When editing `download_images_legacy.go`, keep these points in mind:
...
```powershell
go mod init download_images_legacy
go mod tidy
```
...
```powershell
go run .\download_images_legacy.go
```
